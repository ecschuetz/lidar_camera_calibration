...
'''
def project_point_cloud(velodyne, velodyne2, img_msg, image_pub):
    # Read image using CV bridge
    try:
        img = CV_BRIDGE.imgmsg_to_cv2(img_msg, 'bgr8')
    except CvBridgeError as e:
    ...
    try:
        ...
        transform = TF_BUFFER.lookup_transform('world', 'velodyne2', rospy.Time())
        velodyne2 = do_transform_cloud(velodyne2, transform)
    except tf2_ros.LookupException:
        pass

    # Extract points from message
    points3D_left = ros_numpy.point_cloud2.pointcloud2_to_array(velodyne)
    points3D_right = ros_numpy.point_cloud2.pointcloud2_to_array(velodyne2)
    points3D = np.concatenate((points3D_left,points3D_right))
...
Outputs: None
'''
def callback(image, camera_info, velodyne,velodyne2, image_pub=None):
    ...
    if PROJECT_MODE:
        project_point_cloud(velodyne,velodyne2, image, image_pub)
...
Outputs: None
'''
def listener(camera_info, image_color, velodyne_points, velodyne2_points, camera_lidar=None):
    # Start node
    ...
    rospy.loginfo('PointCloud2 topic: %s' % velodyne2_points)
    ...
    velodyne2_sub = message_filters.Subscriber(velodyne2_points, PointCloud2)
    ...
    ats = message_filters.ApproximateTimeSynchronizer(
        [image_sub, info_sub, velodyne_sub,velodyne2_sub], queue_size=5, slop=0.1)
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == '--calibrate':
        ...
        velodyne2_points = '/points_2_2/points_raw'
        camera_lidar = None
        PROJECT_MODE = False
    # Projection mode, run from launch file
        ...
        velodyne2_points = rospy.get_param('velodyne2_points_topic')
    ...
    # Start subscriber
    listener(camera_info, image_color, velodyne_points, velodyne2_points, camera_lidar)
